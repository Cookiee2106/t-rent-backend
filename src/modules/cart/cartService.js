...
async function themVaoGioHangService(khachHangId, body) {
  ...
  const mau = await cartRepository.layMauThietBiDangHienThi(mau_thiet_bi_id);
...
async function capNhatSanPhamService(khachHangId, itemId, body) {
  const item = await cartRepository.layItemThuocKhachHang(khachHangId, itemId);
